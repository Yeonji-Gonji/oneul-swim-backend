...
function makeService(pool: { findMany: jest.Mock }) {
  const prisma = { pool } as unknown as ConstructorParameters<
    typeof PoolsService
  >[0];
...
describe('PoolsService.getPools', () => {
  it('DB 에 Pool 이 있으면 source=db 로 조립한다(요금은 pool.fees)', async () => {
      ...
      findMany: jest.fn().mockResolvedValue([
        {
          ...
          freeSwim: { sessions: [] },
          lessons: {},
          fees: { full: { 성인: 3300 }, half: { 성인: 1650 } },
          dataStatus: 'full',
          createdAt: new Date(),
    ...
    expect(payload.pools[0].fees?.full.성인).toBe(3300);
    // 무중단 이행용 top-level 호환 요금표도 채워진다
